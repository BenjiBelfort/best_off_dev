...
function Callout({ children }) {
  return (
    <div className="my-4 md:mx-8 rounded-xs border-2 border-yellow-200/7 bg-yellow-50/4 p-4 text-yellow-50">
      {children}
    </div>
  )
}
...
const components = {
  ...
  h2: (p) => <h2 {...p} className="md:mt-8 mb-3 text-2xl font-semibold tracking-tight" />,
  h3: (p) => <h3 {...p} className="mt-6 mb-2 text-xl font-semibold" />,
  h4: (p) => <h4 {...p} />,
  p:  (p) => <p  {...p} className="mb-4 leading-relaxed" />,
  a:  A,
  ul: (p) => <ul {...p} className="md:mx-8 mb-4 list-disc pl-6" />,
  ol: (p) => <ol {...p} className="mb-4 list-decimal pl-6" />,
  blockquote: (p) => <blockquote {...p} className="md:mx-8 my-4 border-l-4 border-white/30 pl-4 italic" />,
  Separator,
  Callout,
}
